...
启动日是仅面向当前苹果芯片电脑的本地智能工作系统。二维冰晶猫桌宠是常驻入口，轻面板负责快速输入和当前行动，完整工作台负责计划、依赖、权限、进度、成果与历史记录。

![桌宠效果](artifacts/desktop-pet.png)

当前桌宠窗口为 120 × 120 像素，完整显示粉彩猫咪、晶石皇冠与冰晶底座。内部使用 900 × 900 高清画布，并通过纯色键控形成透明背景，缩放后的二维边缘保持清晰。拖到桌面右侧会自动切换成探头形态，鼠标移到桌宠左上区域可点击切换按钮手动进入或退出。

成品只保留苹果芯片程序，不包含英特尔版本；桌宠包含轻微呼吸、上下浮动、点击弹跳和拖拽反馈，不再加载三维模型、毛发或灯光。
