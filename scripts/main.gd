...
func _ready() -> void:
	...
	interaction.setup(window_controller, animator, integrated)
	if integrated:
...
func _add_camera_and_lights() -> void:
	...
	key_light.light_color = Color("fffce2")
	key_light.light_energy = 4.4
	...
	fill_light.light_color = Color("f2f7d2")
