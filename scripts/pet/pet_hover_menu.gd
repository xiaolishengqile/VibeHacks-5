...
func setup() -> void:
	...
	for spec in BUTTON_SPECS:
		...
		window.focus_exited.connect(func() -> void: focus_moved.emit())
		...
		button.pressed.connect(func() -> void: action_requested.emit(String(spec["event"])))
...
static func button_rects(pet_position: Vector2i, work_area: Rect2i) -> Array[Rect2i]:
	...
	for spec in BUTTON_SPECS:
		var desired: Vector2i = pet_position + spec["offset"]
